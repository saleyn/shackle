...
noop_dispatcher_test() ->
    ...
    Fun = fun() -> result end,
    ?assertEqual(result, shackle_observe_noop:call(test_pool, Fun)),
    ?assertEqual(result, shackle_observe_noop:cast(test_pool, Fun)),
    ?assertEqual(result, shackle_observe_noop:connect(test_pool, Fun)),
    ?assertEqual(result, shackle_observe_noop:timeout(test_pool, Fun)),
    ?assertEqual(result, shackle_observe_noop:error(test_pool, Fun)).
...
span_dispatcher_test() ->
    ...
    ?assert(erlang:function_exported(Module, call, 2)),
    ?assert(erlang:function_exported(Module, cast, 2)),
    ?assert(erlang:function_exported(Module, connect, 2)),
    ?assert(erlang:function_exported(Module, timeout, 2)),
    ?assert(erlang:function_exported(Module, error, 2)).
